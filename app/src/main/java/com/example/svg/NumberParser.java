 ...
 *  SVGNumber --> integer ([Ee] integer)? | [+-]? [0-9]* "." [0-9]+ ([Ee] integer)?
 * 'integer' --> [+-]? [0-9]+
 * tq : [] = un et selement un ,
 *      ? = un au plus
 *      + = un au moins
 *      * un ou plusieurs
 *
...
class NumberParser
{
    ...
    float  parseNumber(String input, int startpos, int len)
    {
        ...
        // pour eliminer les grands elements
        // le plus grand float  (2 - 2^23) . 2^127 ~== 3.4e38
        // plus grand  negative float is 2^-149 ~== 1.4e-45
